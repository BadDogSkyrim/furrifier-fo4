...
# --- Build number (a separate counter, not part of __version__) -----------
#
# Mirrors furrify_skyrim.spec. The version in src/furrifier_fo4/__init__.py is
# hand-edited and semantic; the build number is a plain integer that increments
# per build and restarts at 0 on a new version line, so "1.2.0 build 7" names
# exactly one kit. It used to be the third version component, which meant a
# rebuild looked like a patch release.
...
_init_src = (_spec_root / 'src' / 'furrifier_fo4' / '__init__.py').read_text(
    encoding='utf-8')
_m = _re.search(r'^__version__\s*=\s*["\']([^"\']+)["\']', _init_src, _re.M)
if not _m:
    raise SystemExit(
        'could not parse __version__ from src/furrifier_fo4/__init__.py')
_version = _m.group(1)
...
(_spec_root / 'src' / 'furrifier_fo4' / '_build_stamp.py').write_text(
    '"""Generated by furrify_fo4.spec at build time. Do not edit."""\n'
    f'BUILD = {_build_no}\n',
    encoding='utf-8')
print(f'Build number: v{_version} build {_build_no}')

_BUILD_HIDDEN = ['furrifier_fo4._build_stamp']
